# 👋 Hi, I'm Lobov Vitalii

🏗 Architect \| Backend Engineer (Go, Distributed Systems)\
🌍 Open for international opportunities & freelance\
📍 Moscow\
📧 lobov.vitaliy@gmail.com

------------------------------------------------------------------------

## 🚀 About Me

Architect with **20+ years in IT**, specializing in **fintech &
government-scale systems**.

I design and build scalable distributed systems with focus on: -
high-load backend services - data pipelines (streaming & batch) -
observability & reliability - long-term system evolution

💡 Core domain: **Federal Treasury & accounting systems
(country-scale)**

------------------------------------------------------------------------

## 🧠 What I Bring

-   🏗 System Architecture (C4, DDD, Event-driven)
-   ⚡ High-load backend (Go)
-   🔄 Data pipelines (Kafka, Airflow, NiFi)
-   📊 Observability-first design
-   🔗 Enterprise integrations

------------------------------------------------------------------------

## 🛠 Tech Stack

### Backend

![Go](https://img.shields.io/badge/-Go-000?&logo=go)
![SQL](https://img.shields.io/badge/-SQL-000?&logo=postgresql)
![ClickHouse](https://img.shields.io/badge/-ClickHouse-000)
![gRPC](https://img.shields.io/badge/-gRPC-000)

### Data & Streaming

![Kafka](https://img.shields.io/badge/-Kafka-000?&logo=apachekafka)
![Apache NiFi](https://img.shields.io/badge/-NiFi-000)
![Apache Airflow](https://img.shields.io/badge/-Airflow-000)
![Redis](https://img.shields.io/badge/-Redis-000?&logo=redis)

### Infrastructure

![Docker](https://img.shields.io/badge/-Docker-000?&logo=docker)
![Kubernetes](https://img.shields.io/badge/-Kubernetes-000?&logo=kubernetes)
![Terraform](https://img.shields.io/badge/-Terraform-000?&logo=terraform)
![Helm](https://img.shields.io/badge/-Helm-000)

### Observability

![Prometheus](https://img.shields.io/badge/-Prometheus-000?&logo=prometheus)
![Grafana](https://img.shields.io/badge/-Grafana-000?&logo=grafana)

------------------------------------------------------------------------

## 🚀 Featured Work

🔹 Kafka → REST Bridge\
🔹 Data Platform (Kafka + NiFi)\
🔹 Observability Systems\
🔹 Auth (Keycloak + PKCE)

------------------------------------------------------------------------

## 🧩 Architecture Mindset

``` text
Scalability → Observability → Reliability → Evolvability
```

> "Good architecture is the ability to change the system safely."

------------------------------------------------------------------------

## 🌍 Open For

-   💼 Freelance (Architecture / Backend)
-   🌐 Remote international roles
-   🧠 Consulting (System design, scaling)

------------------------------------------------------------------------

## 📊 GitHub Stats


<img src="https://github-readme-stats.vercel.app/api?username=LobovVit&show_icons=true&theme=tokyonight" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=LobovVit&theme=tokyonight" />


------------------------------------------------------------------------

## 📫 Contact

📧 lobov.vitaliy@gmail.com

